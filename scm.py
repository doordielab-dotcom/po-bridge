        ...
        with col_clear:
            if st.button("🗑️ 전체 데이터 삭제", type="secondary"):
                if st.session_state.get('confirm_delete'):
                    supabase.table("purchase_orders").delete().neq("id", -1).execute() # 모든 데이터 삭제
                    st.session_state['confirm_delete'] = False
                    st.success("모든 데이터가 삭제되었습니다.")
                    time.sleep(1)
                    st.rerun()
                else:
                    st.warning("정말 모든 데이터를 삭제하시겠습니까? 버튼을 한번 더 누르면 삭제됩니다.")
